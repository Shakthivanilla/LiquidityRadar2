...
bg_path = os.path.join(os.path.dirname(__file__), "..", "assets", "bg_sci_fi.jpg")
if os.path.exists(bg_path):
    st.markdown(f'<style>div[data-testid="stAppViewContainer"]{{background-image: url("{bg_path}"); background-size: cover;}}</style>', unsafe_allow_html=True)
...
col4.metric("Updated At", df['updated_at'].max() if 'updated_at' in df.columns else "N/A")
...
if not top.empty:
    ...
    fig = create_liquidity_forecast(top, member_name)
    st.pyplot(fig)
else:
